...
function RankTable({ title, rows, unit }: { title: string; rows: { key: string; count: number; publico: number }[]; unit: string }) {
  return (
    <section>
...
export default async function MiWrappedPage() {
  ...
  const { data } = await supabase
    .from('shows')
    .select('show_date, city, courtesy_count, status, spectacle, theater:theater_id(name), ticket_sales(qty_sold)')
    .is('deleted_at', null)
  ...
  const perShow = shows.map(s => {
    ...
    return { city: s.city ?? '—', theater: s.theater?.name ?? '—', year, spectacle: s.spectacle ?? '—', publico }
  })

  const totalShows = shows.length
  const totalPublico = perShow.reduce((a, s) => a + s.publico, 0)
  const bySpectacle = agg(perShow.map(s => ({ key: s.spectacle, publico: s.publico })))
