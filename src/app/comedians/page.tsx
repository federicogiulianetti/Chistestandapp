...
export default async function ComediansPage({
  ...
  const { data: ensembles } = await supabase
    .from('ensembles')
    .select('id, name, photo_url, ensemble_members(count)')
              ...
              {ensembles.map((e) => {
                ...
                return (
                  <Link
                    key={e.id}
                    href={`/ensembles/${e.id}`}
                    className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 flex items-center gap-4 hover:bg-zinc-800/60 transition"
                  >
                    {e.photo_url ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img
                      ...
                      <div className="w-16 h-16 rounded-lg bg-zinc-800 border border-zinc-700 flex items-center justify-center text-2xl text-gray-500 flex-shrink-0">
                        🎭
                      </div>
                    )}
                    <div>
