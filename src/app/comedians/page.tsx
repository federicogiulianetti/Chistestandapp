...
export default async function ComediansPage({
  ...
  // Solo comediantes que trabajan en solitario
  const { data: comedians } = await supabase
    .from('comedians')
    .select('id, stage_name, full_name, photo_url, city, is_active, instagram_handle, facebook_url, twitter_handle, tiktok_handle, youtube_url, spotify_url, website_url')
    .eq('performs_solo', true)
    .order('stage_name', { ascending: true })

  // Elencos con sus miembros
  const { data: ensembles } = await supabase
    .from('ensembles')
    .select('id, name, photo_url, city, is_active, instagram_handle, tiktok_handle, youtube_url, website_url, ensemble_members(count)')
    .order('name', { ascending: true })

  const canManage = profile.role === 'admin'

  const totalComediantes = comedians?.length ?? 0
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <section className="mb-12">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold">Comediantes</h2>
            {canManage && (
              <Link
                href="/comedians/new"
                    ...
                    <tr key={c.id} className="border-b border-zinc-800 last:border-0 hover:bg-zinc-800/30">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <SocialIcon href={c.facebook_url} label="Facebook">
                            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                          ...
                          <SocialIcon href={c.youtube_url} label="YouTube">
                            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                              <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                            </svg>
                          </SocialIcon>
                          <SocialIcon href={c.spotify_url} label="Spotify">
                            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                          ...
                          <SocialIcon href={c.website_url} label="Sitio web">
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold">Elencos</h2>
            {canManage && (
              <Link
                href="/ensembles/new"
                className="px-4 py-2 bg-white text-black font-semibold rounded-md hover:bg-gray-200 transition text-sm"
                    ...
                    return (
                      <tr key={e.id} className="border-b border-zinc-800 last:border-0 hover:bg-zinc-800/30">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <SocialIcon
                              href={e.instagram_handle ? `https://instagram.com/${e.instagram_handle}` : null}
                              label="Instagram"
                            >
                              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                            ...
                            <SocialIcon
                              href={e.tiktok_handle ? `https://tiktok.com/@${e.tiktok_handle}` : null}
                              label="TikTok"
                            >
                              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-5.2 1.74 2.89 2.89 0 012.31-4.64 2.93 2.93 0 01.88.13V9.4a6.84 6.84 0 00-1-.05A6.33 6.33 0 005.8 20.1a6.34 6.34 0 0010.86-4.43v-7a8.16 8.16 0 004.77 1.52v-3.4a4.85 4.85 0 01-1.84-.1z" />
                            ...
                            <SocialIcon href={e.youtube_url} label="YouTube">
                              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                              </svg>
                            </SocialIcon>
                            <SocialIcon href={e.website_url} label="Sitio web">
                              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
                              </svg>
                            </SocialIcon>
