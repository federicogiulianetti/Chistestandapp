...
function DirectionsLinks({ value }: { value?: string | null }) {
  ...
  return (
    <div className="flex gap-3 text-xs mt-1">
      <a href={`https://www.google.com/maps/dir/?api=1&destination=${dest}`} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300">📍 Google Maps</a>
      <a href={`https://waze.com/ul?q=${dest}&navigate=yes`} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300">🚗 Waze</a>
    </div>
  )
}
    ...
    <div>
      ...
      {!urls || urls.length === 0 ? (
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {urls.map((u, i) => (
            <img key={i} src={u} alt="" className="w-40 h-40 object-cover rounded-lg border border-zinc-700" />
          ))}
        </div>
      )}
    </div>
  ...
  return (
    <main className="min-h-screen bg-black text-white p-8">
      <div className="max-w-3xl mx-auto space-y-6">
        <div>
          <Link href="/theaters" className="text-gray-400 hover:text-white text-sm">← Teatros</Link>
          <div className="flex items-center justify-between mt-2 gap-4">
            <h1 className="text-3xl font-bold">{t.name}</h1>
            {canManage && (
              <Link href={`/theaters/${t.id}`} className="px-4 py-2 bg-white text-black font-semibold rounded-md hover:bg-gray-200 transition text-sm flex-shrink-0">
                Editar
              </Link>
            )}
          </div>
        ...
        <Section title="🎭 Datos generales">
          <Row label="🏙️ Ciudad" value={t.city} />
          <Row label="🗺️ Provincia" value={t.province} />
          ...
          {t.maps_url && (
            <Row label="🔗 Google Maps" value={<a href={t.maps_url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300">Abrir</a>} />
          )}
          {t.address && (
            <div className="pt-3">
        ...
        <Section title="🅿️ Estacionamiento">
          ...
          {!t.parking_at_door && parkings.length > 0 && (
            <div className="pt-2 space-y-2">
              <p className="text-gray-400 text-sm">🅿️ Estacionamientos cercanos</p>
              {parkings.map((addr, i) => (
                <div key={i}>
        ...
        <Section title="🎟️ Boletería y ticketera">
          ...
          <BoolRow label="🎟️ Entradas físicas" value={t.needs_physical_tickets} />
          <BoolRow label="🎫 Ticketera propia" value={t.own_ticketera_allowed} />
          <Row label="🏷️ Ticketera del teatro" value={t.ticketera_name} />
        ...
        <Section title="🔊 Técnica">
          ...
          <BoolRow label="📽️ Proyector" value={t.has_projector} />
          <BoolRow label="🖥️ Pantalla" value={t.has_screen} />
          <BoolRow label="🪑 Banqueta en escenario" value={t.has_banqueta} />
          <BoolRow label="❄️ Aire acondicionado" value={t.has_ac} />
          <BoolRow label="🎚️ Operador" value={t.has_operator} />
          ...
          <Row label="📄 Rider" value={t.rider_url ? <a href={t.rider_url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300">Abrir</a> : null} />
        </Section>
        ...
        <Section title="📣 Cartelería">
          ...
          <BoolRow label="🖼️ Logo del teatro en piezas" value={t.requires_theater_logo} />
          {t.theater_logo_url && (
            <Row label="🔗 Logo" value={<a href={t.theater_logo_url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300">Ver logo</a>} />
          )}
        </Section>
        ...
        <Section title="💰 Acuerdo económico">
          ...
          <BoolRow label="🛡️ Seguro ART" value={t.requires_art_insurance} />
          <BoolRow label="📝 Pasan Argentores" value={t.passes_argentores} />
          <Row label="🏛️ Impuestos" value={t.municipal_taxes} />
          <BoolRow label="🏨 BDX hotel" value={t.bdx_hotel} />
          <BoolRow label="✈️ BDX pasajes" value={t.bdx_transport} />
        ...
        <Section title="🚨 Emergencia y servicios">
          {t.nearest_police_station && (
            <div className="py-1.5">
              <p className="text-gray-400 text-sm">🚓 Comisaría</p>
          ...
          {t.nearest_consulate && (
            <div className="py-1.5">
              <p className="text-gray-400 text-sm">🏛️ Consulado</p>
          ...
          {restaurants.length > 0 && (
            <div className="py-1.5 space-y-2">
              <p className="text-gray-400 text-sm">🍽️ Restaurantes cercanos</p>
