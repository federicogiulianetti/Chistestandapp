    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-surface border border-line rounded-lg p-12 text-center">
          <p className="text-muted">No hay hoteles que coincidan con la búsqueda.</p>
        </div>
      ) : (
            ...
            <tbody>
              {filtered.map(h => (
                <tr key={h.id} className="border-b border-line last:border-0 hover:bg-surface-2">
                  ...
                  <td className="px-4 py-3 text-muted text-sm whitespace-nowrap"><CountryFlag country={h.country} /></td>
                  <td className="px-4 py-3 text-muted text-sm whitespace-nowrap">{h.province || '—'}</td>
                  <td className="px-4 py-3 text-muted text-sm whitespace-nowrap">{h.city || '—'}</td>
                  <td className="px-4 py-3 text-sm whitespace-nowrap">{h.has_canje ? '🤝 Sí' : '—'}</td>
                  <td className="px-4 py-3 text-muted text-sm whitespace-nowrap">{h.pref_count > 0 ? `🌟 ${h.pref_count}` : '—'}</td>
                  ...
                  <td className="px-4 py-3 text-right whitespace-nowrap">
                    <div className="flex gap-3 justify-end">
                      <Link href={`/hotels/${h.id}/ver`} className="text-brand hover:underline text-sm">Ver</Link>
                      {canManage && (
                        <Link href={`/hotels/${h.id}`} className="text-body hover:underline text-sm">Editar</Link>
                      )}
                    </div>
                  </td>
                </tr>
